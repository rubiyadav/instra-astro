...
module.exports.UserFeedback = async (req, res) => {
  ...
  try {
    if (!(UserId && Feedback)) {
      ...
      if (NewUserFeedback) res.status(200).json({ message: "UserFeedback Send", data: NewUserFeedback, status: true, });
      res.status(400).json({ message: "UserFeedback  not send", status: false });
...
module.exports.ViewAllFeedback = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'See All Feedback', Feedback: SendFeedback, status: true })
  } catch (error) {
    console.log(error);
  }
};
